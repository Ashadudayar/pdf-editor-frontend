...
import { Upload, ArrowLeft, Download, Trash2, Check } from 'lucide-react';
...
export default function RemovePagesPage() {
  ...
  const handleRemove = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'https://positive-creativity-production.up.railway.app/api'}/documents/${documentId}/remove-pages/`,
        {
          method: 'POST',
  ...
  const handleDownload = () => {
    if (resultUrl) {
      ...
      link.href = resultUrl;
      link.download = 'removed_pages.pdf';
  ...
  const remainingPages = numPages - selectedCount;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Remove Pages</h1>
          ...
          <div className="text-center">
            <div className="bg-white rounded-2xl shadow-lg p-12 mb-6">
              ...
              <p className="text-gray-600 mb-8">
                Removed {selectedCount} page{selectedCount > 1 ? 's' : ''} • {remainingPages} page{remainingPages > 1 ? 's' : ''} remaining
              </p>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8">
            <div className="border-2 border-dashed border-gray-300 rounded-xl p-12 text-center hover:border-red-400 transition-colors">
              ...
              <label htmlFor="file-upload" className="cursor-pointer">
                ...
                <p className="text-sm text-gray-500">Upload a PDF to remove pages</p>
              </label>
            </div>
          </div>
        ) : (
          ...
          <div className="space-y-6">
              ...
              {selectedCount > 0 && (
                <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg">
                  ...
                  <p className="text-xs text-gray-600 mt-2">
                    {selectedCount} page{selectedCount > 1 ? 's' : ''} selected • {remainingPages} will remain
                  </p>
                </div>
              )}
            </div>
                ...
                {pages.map((page) => (
                  <div
                    ...
                    className={`relative cursor-pointer rounded-xl border-2 p-3 transition-all hover:shadow-lg
                      ${page.selected ? 'border-red-500 bg-red-50' : 'border-gray-200 hover:border-red-300'}`}
                  >
                    {/* Red X Overlay when selected */}
                    {page.selected && (
                      <div className="absolute inset-0 bg-red-500 bg-opacity-20 rounded-xl flex items-center justify-center z-10">
                    ...
                    {/* Page Number */}
                    <p className={`text-center text-sm font-medium ${page.selected ? 'text-red-600' : 'text-gray-900'}`}>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              <button
                ...
                disabled={loading || selectedCount === 0 || selectedCount === numPages}
                className="w-full px-8 py-4 bg-red-600 text-white rounded-xl hover:bg-red-700 transition-colors font-semibold text-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
